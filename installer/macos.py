...
def get_install_dir():
    return Path.home() / "Library" / "Application Support" / "STRCT"
...
def create_symlink(install_dir):
    ...
    try:
        ...
        # If the symlink exists but points to the wrong location, or if it's not a symlink
...
def post_install_message(install_dir):
    ...
    print("Please restart your terminal or run 'source ~/.zshrc' (for Zsh) or 'source ~/.bash_profile' (for Bash) to use STRCT.")